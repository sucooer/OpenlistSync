...
func Parse(sub string, args []string) (*Config, error) {
	...
	fs.StringVar(&flInclude, "include-ext", "", "include only files matching these extensions, comma separated. Bare suffixes (mp4, .txt) match by extension; full globs (*.mp4,*[Bb]ak) are also accepted.")
	fs.StringVar(&flExclude, "exclude-ext", "", "skip files matching these extensions, comma separated. Same format as --include-ext.")
